...
<a  data-sveltekit-preload-data="hover" in:fade="{{ duration:1000, ease:'circ' }}"  on:mouseenter={enter} on:click={enter} on:mouseleave={leave} class='p-6 bg-gray-400 text-gray-800 text-center rounded-md shadow-md hover:shadow-lg flex flex-col items-center' href='/player/{id.replace(",", "")}'>
    {#if hovering && goals}
